...
describe('Generate board cells', () => {
  it('Should returns a valid number of cells', () => {
    ...
    expect(nbOfGeneratedCells).toBe(
      nbOfCellsInGameState + nbOfBorderCellsAdded,
    );
  });
  ...
  it('Should returns valid data for first playable cell', () => {
    ...
    expect(firstPlayableCell.withoutBorderCoordinates).toStrictEqual({
      x: 0,
      y: 0,
    });
  });
  it('Should returns valid data for last playable cell', () => {
    ...
    expect(lastPlayableCell.withoutBorderCoordinates).toStrictEqual({
      x: 5,
      y: 5,
    });
  });
